...
def view():
    ...
    viewmodel.get_notes_all.return_value = "Note Content 0"
    view = View(viewmodel)
    yield view
    app.quit()
    del app
...
def test_header_init_ui():
    ...
    viewmodel.get_notes_all.return_value = view_data_return
...
def test_header_update_text_on_emit():
    ...
    assert header.text() == viewmodel.get_notes_all()
    assert header.text() == f"{note_1_data['content']} 0"
    assert spy.count() == 0

    with patch("epii.view_model.ViewModel.get_notes_all") as mock_get_notes_all:
        new_data = "New Note Content"
        mock_get_notes_all.return_value = new_data
